...
$.widget("ui.mouse", {
	...
	_mouseDown: function(event) {
		...
		//***************************
		//START: CUSTOM CODE ADDED BY APPEXCHANGE TEAM
		//***************************
		if ( this.element[0] && this.element[0].setCapture ){ 
			this.element[0].setCapture(); 
		}
		//***************************
		//END: CUSTOM CODE ADDED BY APPEXCHANGE TEAM
		//***************************
...
(function($) {
    ...
    var jScroll = function($e, options) {
        ...
        var _data = $e.data('jscroll'),
            ...
            _$next = $(_options.nextSelector).first(),
            _$window = $(window),
            _$document = $(document),
            _$scroll = $(window); //_isWindow ? $(window) : $e,
        ...
        function _nextWrap($next) {
            $next.addClass('jscroll-next-parent').hide();
        ...
        function _checkNextHref(data) {
            ...
            var nextLink = $(_options.nextSelector).last();
            
            if (!data.nextHref || !nextLink.attr('href')) {
        ...
        function _load() {
            ...
            return _checkNextHref(data) && $e.animate({scrollTop: $inner.outerHeight()}, 0, function() {
                $inner.find('div.jscroll-added').last().load(data.nextHref, function(r, status, xhr) {
                    var $next = $(_options.nextSelector).last();
